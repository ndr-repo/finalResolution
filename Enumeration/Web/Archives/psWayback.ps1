...
Invoke-RestMethod -Method Get -Uri "http://web.archive.org/cdx/search/cdx?url=$targetDomain/*&output=json&fl=original&collapse=urlkey" | Select-Object -ExpandProperty SyncRoot | grep -o -P "^http.*" | Out-File $resultsFile -Encoding utf8 -Force
Write-Host " "
